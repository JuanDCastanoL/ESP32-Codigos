...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, CANAL_ADC, &config));

    //-------------ADC1 Calibration Init---------------//
    adc_cali_handle_t adc1_cali_chan4_handle = NULL;
    bool do_calibration1_chan4 = example_adc_calibration_init(ADC_UNIT_1, CANAL_ADC, ATTEN_ADC, &adc1_cali_chan4_handle);

    while(1){
        
        ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, CANAL_ADC, &adc_raw[0]));
